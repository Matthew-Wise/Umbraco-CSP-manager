...
- Header construction **threw an exception** and `FailureBehavior` is left at its default, `FailOpen` — check your logs for a `Failed to construct CSP header for {Path}` error. Set `FailureBehavior` to `FailClosed` in `appsettings.json` to apply a minimal `default-src 'self'` fallback policy to the frontend instead of no header while you fix the underlying error. See [Configuration](features/configuration) for details.
