...
import type { CspDefinitionSource } from "../../../api/index.js";
import { UmbCspManagerWorkspaceContext, UMB_CSP_MANAGER_WORKSPACE_CONTEXT, type WorkspaceState } from "../../context/workspace.context.js";
  ...
  override render() {
    ...
    return html`
      <uui-box headline="Sources">
        <p>Manage CSP sources and their directives. Each source defines what content can be loaded and which CSP directives apply to it.</p>
        
        ${this._workspaceState.definition.sources.length === 0 ? html`
          <div class="empty-state">
            <p>No sources configured yet. Add a source to get started.</p>
          </div>
        ` : html`
          ${this._workspaceState.definition.sources.map((source, index) => html`
            <div class="source-item">
              <div class="source-header" @click=${() => this._toggleSourceExpansion(index)}>
                <div class="source-header-content">
                  ...
                  <span class="source-summary">${source.directives.length} directive${source.directives.length !== 1 ? 's' : ''}</span>
                </div>
                <div class="source-header-actions">
                  <uui-action-bar>
                    <uui-button look="outline" color="default" type="button" label="Copy" @click=${(e: Event) => { e.stopPropagation(); this._handleCopySource(source, index); }}>
                      <uui-icon name="copy"></uui-icon>
                    </uui-button>
                    <uui-button look="outline" color="danger" type="button" label="Delete" @click=${(e: Event) => { e.stopPropagation(); this._handleDeleteSource(source); }}>
                      <uui-icon name="delete"></uui-icon>
                    </uui-button>
                  </uui-action-bar>
                  <uui-symbol-expand 
                    .open=${this._expandedSources.has(index)}
                    class="expand-icon">
                  </uui-symbol-expand>
                </div>
              </div>
              ...
              <div class="source-content" ?hidden=${!this._expandedSources.has(index)}>
                <div class="source-fields">
                  ...
                  <uui-form-layout-item>
                    ...
                    <span slot="description">Select which Content Security Policy directives this source applies to</span>
                    <div class="directives-grid">
                      ${this._workspaceState.availableDirectives.map(directive => html`
                        <uui-checkbox 
        ...
        <uui-button label="Add Source" look="primary" style="margin-top: var(--uui-size-space-4)" @click=${this._handleAddSource}>
