...
public static class UmbracoBuilderExtensions
{
	public static IUmbracoBuilder AddCspManager(this IUmbracoBuilder builder)
	{
		...
		if (builder.Services.FirstOrDefault(x => x.ServiceType == typeof(ICspService)) is not null)
		{
			return builder;
		}
		...
		builder.Services.AddAuthorizationBuilder().AddPolicy(Constants.AuthorizationPolicies.SectionAccess,
			policy =>
			{
				...
				policy.Requirements.Add(new CspManagerApplicationRequirement(Constants.SectionAlias));
